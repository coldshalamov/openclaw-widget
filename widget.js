// OpenClaw — living widget
// Renders whatever the agent has pushed to content/art-v2.json on main.
// Designed so the agent can express itself with rich primitives:
// palettes, glyphs, animations, generators, pixel art, feeds, schedules.

const CONFIG = {
  CONTENT_URL: "https://raw.githubusercontent.com/coldshalamov/openclaw-widget/main/content/art-v2.json",
  CACHE_FILE: "openclaw_v3_cache.json",
  REFRESH_SECONDS: 600
};
...
function padRight(s, w) { return s.length >= w ? s.slice(0, w) : s + " ".repeat(w - s.length); }
function center(s, w) {
...
// generative: starfield with optional constellation overlay
...
// rainbow gradient text: each character colored
function renderRainbow(stack, scene, palette, sz) {
  var lines = (scene.content || scene.text || "").split("\n");
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (!line.length) { stack.addText(" ").font = Font.systemFont(sz.fs, "monospaced"); continue; }
...
// pixel art via DrawContext: agent pushes a color grid, we render a bitmap
...
function renderMarquee(stack, scene, palette, sz) {
  ...
  var preLines = Math.floor(sz.rows / 2) - 2;
  for (var i = 0; i < preLines; i++) stack.addText(" ").font = Font.systemFont(sz.fs, "monospaced");
...
// constellation: agent's feed → a starmap, brightness = recency
function renderConstellation(stack, scene, palette, sz, content) {
  ...
  var now = Date.now();
  var horizon = 14 * 24 * 3600 * 1000; // two weeks
  var glyphs = ["·", "∙", "*", "✦", "★"];
  ...
  // sprinkle background stars for atmosphere
  ...
  // headline = newest feed entry
  if (feed.length) {
    var newest = feed[0];
    addMono(stack, "» " + (newest.msg || "").slice(0, cols - 2), palette[0], sz.fs);
  }
  for (var j = 0; j < rows; j++) {
    var col = rampColor(palette, j / Math.max(1, rows - 1));
    addMono(stack, grid[j].join(""), col, sz.fs);
  }
}

// composite: a structured dashboard scene
...
async function fetchContent() {
  ...
  var cachePath = fm.joinPath(fm.documentsDirectory(), CONFIG.CACHE_FILE);
  try {
...
function offlineContent() {
  return {
    version: "offline",
    agent: { name: "openclaw", mood: "offline" },
    primitives: { palettes: { day: ["#ff6b6b", "#ffa500", "#feca57"] } },
    scenes: [{
      type: "message",
      text: "  no signal  \n  waiting…  ",
...
async function createWidget() {
  ...
  w.refreshAfterDate = new Date(Date.now() + CONFIG.REFRESH_SECONDS * 1000);
...
if (config.runsInWidget) {
  ...
  else if (fam === "extraLarge") widget.presentExtraLarge();
